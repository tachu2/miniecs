...
### 1. アプリケーション (Go)
*   [ ] **実装:** `net/http` を使用した軽量HTTPサーバー (`./app/main.go`)
...
#### 基本設定 & ネットワーク (VPC)
*   [ ] **Provider設定:** AWSプロバイダーとバージョンの固定
*   [ ] **VPC作成:** CIDRブロック (例: `10.0.0.0/16`)
*   [ ] **サブネット作成:**
...
*   [ ] **ECRリポジトリ:** Dockerイメージの格納先
*   [ ] **ECSクラスター:** Fargateキャパシティプロバイダー設定
...
#### ロードバランシング (ALB)
*   [ ] **ALB本体:** インターネット向け (Internet-facing)、パブリックサブネットに配置
*   [ ] **ターゲットグループ:** Goアプリのヘルスチェック設定 (例: `GET /`)
*   [ ] **リスナー:** ポート80 (HTTP) でのリクエスト受け付け
